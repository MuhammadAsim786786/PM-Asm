module.exports = [
"[project]/.next-internal/server/app/api/compareFromSources/route/actions.js [app-rsc] (server actions loader, ecmascript)", ((__turbopack_context__, module, exports) => {
...
"[project]/app/api/compareFromSources/route.js [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
const BOOK_DIRS = [
    __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].join(process.cwd(), "data"),
    __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].join(process.cwd(), "public", "data")
];
function safe(s) {
    return (s ?? "").toString();
}
...
async function findBooksDir() {
    for (const d of BOOK_DIRS){
        try {
            const st = await __TURBOPACK__imported__module__$5b$externals$5d2f$fs__$5b$external$5d$__$28$fs$2c$__cjs$29$__["default"].promises.stat(d);
...
async function loadBooks(dir) {
    const files = await __TURBOPACK__imported__module__$5b$externals$5d2f$fs__$5b$external$5d$__$28$fs$2c$__cjs$29$__["default"].promises.readdir(dir);
    const results = [];
    for (const f of files){
        ...
        try {
            const raw = await __TURBOPACK__imported__module__$5b$externals$5d2f$fs__$5b$external$5d$__$28$fs$2c$__cjs$29$__["default"].promises.readFile(full, "utf8");
            results.push({
                fileName: f,
                data: JSON.parse(raw)
            });
...
async function POST(req) {
    try {
        ...
        if (!topic) return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            success: false,
        ...
        if (!dir) return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            success: false,
        ...
        for (const b of books){
            ...
            if (hits.length > 0) matched.push({
                ...b,
                matches: hits.slice(0, MAX_SECTIONS)
            });
        }
        // fallback: if exact match not found, include all books
        if (matched.length === 0) {
            matched.push(...books.map((b)=>({
                    ...b,
                    matches: b.sections.slice(0, 1)
                })));
        }
        // === per-book summaries ===
        const summaries = [];
...
`;
            ...
            const summary = res?.choices?.[0]?.message?.content?.trim() ?? "(no summary)";
            summaries.push({
                bookId: b.bookId,
                title: b.title,
                summary
...
`;
        const agg = await callGroq([
            {
                role: "system",
                content: "You are an expert in comparative project management standards."
            },
            {
                role: "user",
        ...
        const finalComparison = agg?.choices?.[0]?.message?.content?.trim() ?? "No comparison available.";
...
//# sourceMappingURL=%5Broot-of-the-server%5D__3f57d6d9._.js.map